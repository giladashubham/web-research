...
from webresearch.types import WorkflowInput, WorkflowResult, WorkflowMetadata, TokenUsage
...
def test_run_creates_default_event_log(monkeypatch, tmp_path) -> None:
    ...
    data = json.loads("\n".join([line for line in log_files[0].read_text().splitlines() if not line.strip().startswith("//")]))
...
def test_run_with_explicit_events_out_file(monkeypatch, tmp_path) -> None:
    ...
    result = CliRunner().invoke(app, ["run", "query", "fake", "--quiet", "--events-out", str(log_path)])
    
    assert result.exit_code == 0
    assert log_path.exists()
    
    data = json.loads("\n".join([line for line in log_path.read_text().splitlines() if not line.strip().startswith("//")]))
...
def test_run_with_explicit_events_out_dir(monkeypatch, tmp_path) -> None:
    ...
    result = CliRunner().invoke(app, ["run", "query", "fake", "--quiet", "--events-out", str(log_dir)])
