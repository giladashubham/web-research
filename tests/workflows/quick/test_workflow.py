...
QUICK_WORKFLOW_MODULE = "webresearch.workflows.quick.workflow"
...
def _patch_agents(monkeypatch) -> None:
    monkeypatch.setattr(
        f"{QUICK_WORKFLOW_MODULE}.planner_agent",
    ...
    monkeypatch.setattr(
        f"{QUICK_WORKFLOW_MODULE}.official_researcher_agent",
        lambda _depth="quick": _research_agent("official"),
    )
    monkeypatch.setattr(
        f"{QUICK_WORKFLOW_MODULE}.broad_researcher_agent",
        lambda _depth="quick": _research_agent("broad"),
    )
    monkeypatch.setattr(
        f"{QUICK_WORKFLOW_MODULE}.output_agent",
...
async def test_quick_loads_from_registry() -> None:
    assert WORKFLOWS["quick"] is run_quick


async def test_quick_runs_end_to_end_against_mock_model(monkeypatch) -> None:
    _patch_agents(monkeypatch)

    result = await run_quick(WorkflowInput(query="query"))
