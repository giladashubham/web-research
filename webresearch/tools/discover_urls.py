...
async def _collect_from_sitemap(
    ctx: WorkflowContext, base: str, seed: str, found: dict[str, set[str]]
) -> None:
    sitemap_url = base + "/sitemap.xml"
    result = await fetch_url(ctx, sitemap_url)
...
async def _collect_from_homepage(
    ctx: WorkflowContext, seed: str, found: dict[str, set[str]]
) -> None:
    result = await fetch_url(ctx, seed)
    if result.status != "fetched":
        return
...
async def _collect_from_high_value_same_site_pages(
    ...
    for candidate in candidates[:8]:
        result = await fetch_url(ctx, candidate)
        if result.status != "fetched":
            continue
...
async def discover_urls(ctx: WorkflowContext, seed_url: str) -> DiscoveredUrls:
    ...
    one-hop expansion from same-site docs/API subdomains, then targeted canonical
    probes for any still-empty categories.
    """
    ...
    await _collect_from_sitemap(ctx, base, normalized_seed, found)
    await _collect_from_homepage(ctx, normalized_seed, found)
    await _collect_from_high_value_same_site_pages(ctx, normalized_seed, found)
    await _probe_canonical_paths(ctx, base, found)
    await _probe_well_known(ctx, base, found)
